...
The embedded private reader scope is profile `A612-A613-v1`, reader version `1.0.0`. Runtime integration passes the actual application assembly version (`1.5.1` for the installer release under development). New approvals require full independent validation. The one-time revision 1 bootstrap may include only exact documented drivers `616.56` and `616.92`; its catalog rationales preserve their different validation levels and limitations.
